import{FaSearch} from "react-icons/fa"
import { Link } from "react-router-dom";

export default function Header() {
  return (
    <header className='bg-slate-200 shadow-md'>
      <div className='flex justify-between items-center max-w-6xl mx-auto p-1'>
        <Link to="/">
      <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'  >
        <span className='text-slate-500'>Sandhya</span>
        <span className='text-slate-700'>Estate</span>
      </h1>
      </Link>
      <form className='bg-slate-100 p-3 rounded-lg flex  items-center'>
        <input type="text" placeholder='search...' className="bg-transparent focus:outline-none w-24 sm:w-62"></input>
        <FaSearch className="text-slate-400"></FaSearch>
      </form>
      <ul className="flex gap-4">
        <Link to="/">
        <li className="text-slate-700 hidden sm:inline hover:underline ">Home</li></Link>
        <Link to="about">
        <li className="text-slate-700 hidden sm:inline hover:underline ">About</li></Link>
        <Link to="sign-in">
        <li className="sm:inline hover:underline ">Sign in</li></Link>
      </ul>
      </div>
      
    </header>
  )
}
